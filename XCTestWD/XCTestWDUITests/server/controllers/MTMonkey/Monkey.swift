...
public class Monkey {
     ...
     */
    public convenience init(frame: CGRect, bundleid: String) {
        let time = Date().timeIntervalSinceReferenceDate
        let seed = UInt32(UInt64(time * 1000) & 0xffffffff)
        self.init(seed: seed, frame: frame, bundleid: bundleid)
    }
    ...
    /**
     ...
     Should be set to the size of the device being tested.
     */
    public init(seed: UInt32, frame: CGRect, bundleid: String) {
...
//                }
                ...
                if XCTestWDApplication.activeApplication().bundleID != self.bundleID {
                    let path :String? = nil
                    let app: XCUIApplication = XCUIApplication.init(privateWithPath: path, bundleID: self.bundleID)!
                    app.activate()
                }
                action()
            }
