...
extension XCUIElement {
    ...
    func pageSourceToPoint() -> [CGPoint]? {
        ...
        let query = "//XCUIElementTypeButton | //XCUIElementTypeStaticText | //XCUIElementTypeImage "
        let matchSnapShots = XCTestWDXPath.findPointMatchesIn(self.lastSnapshot, query)
    ...
    func descendantsMatchingIdentifier(accessibilityId:String, returnAfterFirstMatch:Bool, isFuzzy:Bool) -> [XCUIElement]? {
        var result = [XCUIElement]()
        ...
        if isFuzzy {
            let predicate = NSPredicate(format: "identifier MATCHES %@ || label MATCHES %@", accessibilityId, accessibilityId)
            query = self.descendants(matching: XCUIElementType.any).matching(predicate)
        } else {
            query = self.descendants(matching: XCUIElementType.any).matching(identifier: accessibilityId)
        }
